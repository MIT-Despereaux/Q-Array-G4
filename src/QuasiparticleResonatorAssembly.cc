...
// 2. Single Resonator Construction (G4MultiUnion)
    ...
    // NEW: Quarter circle continuing the pattern (sweeping 90 degrees from the left side down to the bottom)
    G4Tubs* botQuarterArc = new G4Tubs("ResBQuarterArc", 45.0*CLHEP::um, 55.0*CLHEP::um, dp_groundPlaneDimZ/2.0, 180.0*CLHEP::deg, 90.0*CLHEP::deg);
