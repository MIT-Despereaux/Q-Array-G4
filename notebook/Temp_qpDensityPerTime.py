...
class QuasiparticleAnalyzer:
    ...
    def plot_density(self, df_binned):
        print("Generating Line Plot with Standard Deviation for JJs 4 and 5...")
        if df_binned.empty:
            print("No data available to plot.")
            return

        # Setup figure with full transparency
        fig, ax = plt.subplots(figsize=(14, 7))
        fig.patch.set_alpha(0.0) 
        ax.patch.set_alpha(0.0)  

        target_jjs = [4, 5] 
        custom_colors = ['#ED7B7B', '#215FAC'] # Updated to c_total and c_neutron colors respectively
        rolling_window_size = 10
        
        for idx, jj_id in enumerate(target_jjs):
            if jj_id not in df_binned['Zone #'].unique():
                continue
            ...
            ax.plot(
                times, counts, label=f'JJ {jj_id}', color=color,
        ...
        ax.set_title('Quasiparticle Density in JJ Zones 4 and 5 Over Time')
        ax.set_xlabel('Time (ns)')
        ax.set_ylabel('Unique QP Count per Bin')
        ax.grid(True, linestyle='--', alpha=0.5)
        ax.legend(title="Transmon JJs", bbox_to_anchor=(1.01, 1), loc='upper left')
        plt.tight_layout()
